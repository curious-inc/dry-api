...
test("hash_to_args fallback", function(done){
    ...
    m.hash_to_args({ third: '3', first: '1', second: '2', args: ['third', 'first', 'second'] }, function(err, args){
        eq(args, ['3', '1', '2']);
        done();
    }, true);
...
test("apply_hash fallback", function(done){
    ...
    m.apply(null, { one: '1', three: 3, invalid: 'invalid', args:['one', '', 'three'] }, function(err, first, second, third){
        eq(first, '1');
        eq(second, undefined);
        eq(third, 3);
        done();
