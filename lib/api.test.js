...
function test_api(role, f){
    ...
    var api = new api_class(null, "test_api", test_access_manager());
...
test("user access_token", function(done){
    ...
    api.call("role", { access_token: "user_token" }, args_to_message([]), function(err, result){
        eq(err, null);
        eq(result, args_to_message(["user", ["user"]], null));
        done();
    });
...
test("admin access_token", function(done){
    ...
    api.call("role", { access_token: "admin_token" }, args_to_message([]), function(err, result){
        eq(err, null);
...
test("method throws", function(done){
    ...
    api.call("method_error", null, [1, 2, 3], function(err, result){
