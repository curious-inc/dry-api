...
function api(api_manager, name, access_manager, log){
...
api.prototype.prepare_context = function(context, callback){
    this.bite("prepare_context", [context], callback, this.context_timeout);
};
...
api.prototype.call = function(method_name, context, hash, callback){
    ...
    self.access_manager().get(context.access_token, function(err, security_context){
        if(err){ return callback(err, self.encode_error(err)); }

        context = _.extend(context, security_context);

        self.prepare_context(context, function(err, context){
            if(err){ return callback(err, self.encode_error(err)); }
            ...
            self.find_method(method_name, context, function(err, method){
                ...
                if(self.testing){ 
                    method.testing = true;
                    return method.apply(context, hash, callback);
                }

                try{ return method.apply(context, hash, callback); }
                catch(e){ 
                    var new_err = self.errors().method_error(e);
                    return callback(new_err, self.encode_error(new_err));
                }
            });
        });
    });
