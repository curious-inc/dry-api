...
function api_manager(roles){

    this._apis = {};

    whitelist.init(this);

    var dummy_api = new api_class(this, "dummy", roles);

    this.encode_error = _.bind(dummy_api.encode_error, dummy_api);
    this.roles = _.bind(dummy_api.roles, dummy_api);
...
api_manager.prototype.api = function(api_name, create){
    ...
    if(create){ 
        var api = _.isObject(create) ? create : new api_class(this, api_name, this.roles().hash());
