...
api_manager.prototype.call = function(ns_method_name, context, args, callback){
    ...
    this.find_api(ns_method_name, _.plumb(function(api, api_name, method_name){
        api.call(method_name, context, args, function(err){
            if(err){
                err.api = api;
                err.api_name = api_name;
                err.method_name = method_name;
                return callback(err);
            }else{ return callback.apply(this, arguments); }
        });
    }, callback));
};

api_manager.prototype.whitelisted = function(err){

    if(this._dummy_api.whitelisted(err)){ return(true); }
    else if(err.api){ return err.api.whitelisted(err); }
    else{ return(false); }
};
...
api_manager.prototype.api = function(api_name, create){
    ...
    if(create){ 
        var api = _.isObject(create) ? create : new api_class(api_name, this.access_manager());
