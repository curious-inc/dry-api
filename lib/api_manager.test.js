...
function test_api_manager(role, f){
    ...
    var manager = new api_manager_class();
    manager.context_prepper(test_access_manager().context_prepper());
...
test("user access_token", function(done){
    ...
    api.call("test.role", null, { tags: { access_token: "user_token" }, params: [] }, function(err, result){
        eq(err, null);
        eq(result, args_to_message(["user", ["user"]], null));
        done();
    });
...
test("admin access_token", function(done){
    ...
    api.call("test.role", null, { tags: { access_token: "admin_token" }, params: [] }, function(err, result){
        eq(err, null);
...
test("manager whitelisted good_error", function(done){
    ...
    api.api("test").public("good_error", function(callback){
        return callback(good);
    });

    api.call("test.good_error", null, args_to_message([]), function(err, result){
        eq(_.code(err), "good");
        eq(result.error.code, "good");
        done();
    });
});

test("manager whitelisted bad_error", function(done){

    var api = test_api_manager();

    var bad = _.error("bad", "bad error.");

    api.api("test").public("bad_error", function(callback){
        return callback(bad);
    });

    api.call("test.bad_error", null, args_to_message([]), function(err, result){
        eq(_.code(err), "bad");
        eq(result.error.code, "error");
        done();
    });
});

test("context.method", function(done){

    var api = test_api_manager();

    api.api("test").public("context_method", function(callback){
        return callback(null, this.method);
    });

    api.call("test.context_method", null, _.extend(args_to_message([]), { method: "test.context_method" }), function(err, result){
        eq(err, null);
        _.p("result: ", result);
        eq(result, args_to_message(["test.context_method"], null));
        done();
    });
});

test("whitelisted context good", function(done){
    ...
    api.call("test.echo", null, {}, function(err, result){
        eq(_.code(err), "good");
        eq(result.error.code, "good");
        done();
    });
...
test("whitelisted context bad", function(done){
