...
function manager(roles){

    roles = roles || new api_class().roles().hash();
...
_.hook(manager.prototype);

manager.prototype.roles = _.r("_roles");

manager.prototype.call = function(ns_method_name, context, args, callback){
...
manager.prototype.find_api = function(ns_method_name, callback){
    var self = this;

    if(!ns_method_name){ return callback(self.errors().malformed_call("A call needs an api name and function name")); }

    var ns_method_name = ns_method_name.split(".");
    if(ns_method_name.length <= 1){ return callback(self.errors().malformed_call("A call needs an api name and function name")); }
...
manager.prototype.find_method = function(ns_method_name, context, callback){
...
manager.prototype.api = function(api_name, create){

    if(this._apis[api_name]){ return(this._apis[api_name]); }

    if(create){ 
        var api = new api_class(api_name, this.roles().hash());
...
/*
manager.prototype.load = function(f){
    var m = "";
