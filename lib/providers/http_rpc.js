...
http_rpc.prototype.error_handler = function(err, req, res, next){
    ...
    var whitelisted = this.api_manager().whitelisted(err);

    var client_error = this.api_manager().errors().error();

    if(whitelisted){ client_error = { code: err.code, message: err.message }; }

    var res_hash = {};
    dummy_method.parameters_to_hash([client_error], function(e, hash){ res_hash = hash; });
    res.write(_.stringify(res_hash));
    res.end();

    return this._outside_error_handler(err, whitelisted);
};
...
http_rpc.prototype.route_handler = function(req, res, next){ 
    ...
    _.log.info("calling: ", hash.method);
